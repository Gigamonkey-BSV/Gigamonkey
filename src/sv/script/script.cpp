...
uint64_t CScript::GetSigOpCount (bool fAccurate, bool isGenesisEnabled, bool &sigOpCountError) const {
    ...
    for (auto it {begin_instructions ()}; it != it_end; ++it) {
        ...
        if (it->opcode () == OP_INVALIDOPCODE) break;

        if (opcode == OP_CHECKSIG || opcode == OP_CHECKSIGVERIFY) n++;
            ...
            } else n += MAX_PUBKEYS_PER_MULTISIG_BEFORE_GENESIS;
        }
...
bool IsP2SH (const std::span<const uint8_t> script) {
...
bsv::instruction_iterator CScript::begin_instructions () const {
    return bsv::instruction_iterator {std::span<const uint8_t> {data (), size ()}};
}

bsv::instruction_iterator CScript::end_instructions () const {
    return bsv::instruction_iterator {
        std::span<const uint8_t> {data () + size (), 0}};
}

std::ostream &operator << (std::ostream &os, const CScript &script) {
    for (auto it = script.begin_instructions (); it != script.end_instructions (); ++it)
        os << *it << '\n';
...
size_t CountOp (const std::span<const uint8_t> s, const opcodetype opcode) {
    using namespace bsv;
