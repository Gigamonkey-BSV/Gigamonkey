...
namespace Gigamonkey::Bitcoin::P2P {
	...
	Messages::Message Node::generateVersion(bool initial) {
		...
		boost::shared_ptr<Messages::VersionPayload>
			version = boost::static_pointer_cast<Messages::VersionPayload>(msg.getPayload());
	...
	void Node::readHeader(const boost::system::error_code &ec, std::size_t bytes_transferred) {
		...
		boost::asio::async_read(_socket,
								response_,
								boost::asio::transfer_exactly(header.getPayloadSize()),
