...
TEST_P (Bip39Tests, CheckChecksum) {
    ...
    ASSERT_TRUE (HD::BIP_39::valid (words,std::get<0> (GetParam ()))) << "Checksum should be valid";
    std::replace (words.begin (), words.end (), 'a', 'e');
    std::replace (words.begin (), words.end (), 'o', 'i');
    ASSERT_FALSE (HD::BIP_39::valid (words,std::get<0> (GetParam ()))) << "Checksum should not valid on altered string";
}

INSTANTIATE_TEST_SUITE_P (Bip39,Bip39Tests,::testing::Values(
        /*std::make_tuple(
                HD::BIP_39::language::japanese,
