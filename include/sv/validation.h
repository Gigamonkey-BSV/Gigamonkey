 ...
 */
bool VerifyNewBlock(const CScriptConfig &config,
 ...
 */
bool ProcessNewBlock(const CScriptConfig &config,
                     const std::shared_ptr<const CBlock>& pblock,
 ...
 */
bool ProcessNewBlockHeaders(const CScriptConfig &config,
 ...
 */
bool LoadExternalBlockFile(const CScriptConfig &config, FILE *fileIn,
 ...
 */
bool InitBlockIndex(const CScriptConfig &config);
 ...
 */
void InitScriptCheckQueues(const CScriptConfig& config, boost::thread_group& threadGroup);
 ...
 */
bool GetTransaction(const CScriptConfig &config, const TxId &txid, CTransactionRef &tx,
...
bool IsUAHFenabled(const CScriptConfig &config, const CBlockIndex *pindexPrev);

/** Check if DAA HF has activated. */
bool IsDAAEnabled(const CScriptConfig &config, const CBlockIndex *pindexPrev);

/** Check if Genesis has activated. */
bool IsGenesisEnabled(const CScriptConfig &config, const CBlockIndex *pindexPrev);
/** Check if Genesis has activated.
 * Do not call this overload with height of coin. If the coin was created in mempool, 
 * this function will throw exception.
 */
bool IsGenesisEnabled(const CScriptConfig& config, int32_t nHeight);
/**  Check if Genesis has activated.
 ...
 */
bool IsGenesisEnabled(const CScriptConfig& config, const CoinWithScript& coin, int32_t mempoolHeight);
int GetGenesisActivationHeight(const CScriptConfig& config);
 ...
 */
bool PreciousBlock(const CScriptConfig &config, CValidationState &state,
                   CBlockIndex *pindex);

/** Mark a block as invalid. */
bool InvalidateBlock(const CScriptConfig &config, CValidationState &state,
                     CBlockIndex *pindex);
