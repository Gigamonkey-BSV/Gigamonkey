...
namespace bsv {
    ...
    decode_instruction (std::span<const uint8_t> s) {
        if (s.empty ()) return {OP_INVALIDOPCODE, 0, 0};

        const opcodetype opcode {static_cast<opcodetype> (s[0])};
        if (opcode > OP_PUSHDATA4 || opcode == 0) return std::make_tuple (opcode, 0, 0);
        ...
        if (opcode < OP_PUSHDATA1) {
            if (opcode <= s.size ()) return {opcode, 0, opcode};
            else return {OP_INVALIDOPCODE, 0, 0};
        }

        if (opcode == OP_PUSHDATA1) {
            if (s[0] > s.size () - 1) return {OP_INVALIDOPCODE, 0, 0};
    ...
    class instruction_iterator : public std::iterator<std::forward_iterator_tag, instruction> {
        std::span<const uint8_t> span_;
        instruction instruction_;

        constexpr instruction next (std::span<const uint8_t> s)
        {
    ...
    public:
        constexpr instruction_iterator (std::span<const uint8_t> s) noexcept :
            span_ {s}, instruction_ (next (s)) {}
        ...
        constexpr bool operator == (const instruction_iterator& other) const noexcept {
