...
namespace bsv {
    ...
    void inline serialize (const T &value, O o) {
        if (value == 0) return;
    ...
    bool inline IsMinimallyEncoded (std::span<const uint8_t> span, size_t nMaxNumSize) {
        const size_t size = span.size ();
        if (size > nMaxNumSize) return false;
        ...
        if (size > 0) {
            ...
            if ((span.back () & 0x7f) == 0) {
                ...
                // (big-endian).
                if (size <= 1 || (span[size - 2] & 0x80) == 0) return false;
            }
        }
    ...
    bool inline MinimallyEncode (std::vector<uint8_t> &data) {
